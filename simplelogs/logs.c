...
#ifndef MICRO_SECONDS
//Chineseguuys@AUSA_UBUNTU, 2021-11-26， ADDED FOR PRINT MICRO SECONDS IN LOGS
  char buf[16];
  buf[strftime(buf, sizeof(buf), "%H:%M:%S", ev->time)] = '\0';
#else /* MICRO_SECONDS */
  char buf[64];
  size_t len = strftime(buf, sizeof(buf), "%H:%M:%S", ev->time);
  buf[sprintf(&buf[len], ".%ld", ev->micro_ses) + len] = '\0';
#endif /* MICRO_SECONDS */
#ifdef LOG_USE_COLOR
  fprintf(
